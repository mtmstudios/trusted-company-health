...
import { Award, Users, CheckCircle, Lock, MapPin } from "lucide-react";
  ...
  return (
    <section className="relative mt-16 flex min-h-[calc(100vh-64px)] w-full items-center overflow-hidden py-12 lg:mt-[72px] lg:min-h-[calc(100vh-72px)] lg:py-16">
      ...
      <div className="section-container grid lg:grid-cols-2 gap-12 lg:gap-16 items-center relative z-10">
        {/* Left */}
        <AnimatedSection>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-light-teal px-4 py-1.5 text-sm font-medium text-primary-teal mb-4 lg:mx-0 mx-auto block w-fit text-center">
            <MapPin size={14} /> Ihr bKV-Partner in Stuttgart
          </span>
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold leading-tight mb-6 text-center lg:text-left">
            Betriebliche Kranken­versicherung in Stuttgart
          </h1>
          <p className="text-foreground max-w-xl mb-8 text-base lg:text-lg text-center lg:text-left mx-auto lg:mx-0">
            Die <strong>betriebliche Krankenversicherung (bKV)</strong> ist der smarte Benefit für Stuttgarter Unternehmen. 
            Sichern Sie Ihre Mitarbeiter ab – <strong>steuer- und sozialabgabefrei</strong>. 
            Bereits ab 6&nbsp;€ pro Monat pro Mitarbeiter, ab 3 Mitarbeitenden möglich.
          </p>
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                <motion.div key="s1" {...slideVariants} transition={{ duration: 0.25, ease: "easeInOut" }}>
                  <h3 className="text-xl font-semibold mb-2 text-center">Jetzt kostenlos anfragen – Stuttgart</h3>
                  <p className="text-sm text-muted-foreground text-center mb-6">
                    Wie viele Mitarbeiter hat Ihr Unternehmen in Stuttgart?
                  </p>
              ...
              {step === 3 && (
                <motion.div key="s3" {...slideVariants} transition={{ duration: 0.25, ease: "easeInOut" }}>
                  <h3 className="text-xl font-semibold mb-4 text-center">
                    Fast geschafft! Ihre kostenlose Beratung in Stuttgart anfordern
                  </h3>
                  <div className="flex flex-wrap gap-2 mb-5 justify-center">
                    {answers.size && (
                      <span className="text-xs bg-light-teal text-primary-teal rounded-full px-3 py-1">{answers.size}</span>
                    )}
                    {answers.goal && (
                      <span className="text-xs bg-light-teal text-primary-teal rounded-full px-3 py-1">{answers.goal}</span>
                    )}
                  </div>
                  <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                    <input type="text" placeholder="Name" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="rounded-lg border border-input bg-transparent px-4 py-3 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring transition-all duration-200" />
                    <input type="text" placeholder="Firmenname" required value={formData.firma} onChange={(e) => setFormData({ ...formData, firma: e.target.value })} className="rounded-lg border border-input bg-transparent px-4 py-3 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring transition-all duration-200" />
                    <input type="email" placeholder="E-Mail" required value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="rounded-lg border border-input bg-transparent px-4 py-3 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring transition-all duration-200" />
                    <input type="tel" placeholder="Telefon" value={formData.telefon} onChange={(e) => setFormData({ ...formData, telefon: e.target.value })} className="rounded-lg border border-input bg-transparent px-4 py-3 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring transition-all duration-200" />
                    <button type="submit" className="w-full rounded-full bg-primary text-primary-foreground font-semibold py-3.5 text-sm hover:bg-secondary-teal hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 mt-1">
